...
The feedback view asks five deliberately ordinary-language questions: did the community survive, did it recover, how much was lost, how unstable was it and what represented functions still worked? A generated cause-and-effect trail links the checkpoint fork, first stored resource and population responses, bottleneck and outcome back to the shared timeline. Beside it, a cumulative help panel explains the same result through Curious, Biology and Engine lenses. The lenses share one factual source; the later lenses add mechanism and implementation detail rather than changing the story. A one-slider concept demo explains a relationship without running or modifying the real experiment. See [Education and help](docs/EDUCATION_AND_HELP.md).
...
- `src/lib/analysis` — deterministic checkpoint control/shadow evaluation, metrics, causal steps and hard validity gates.
...
The current comparison is a real branch from one content-hashed checkpoint and verifies exact resume against the uninterrupted run. Hard gates check checkpoint/prefix/branch integrity, finite and non-negative stored state, repeatability and one unsupported-growth pattern. Complete unit-aware matter/energy conservation and accounting for material introduced by prototype floors and caps are still unavailable, so the UI names those missing gates beside the result. “Recovered” remains a declared aggregate-biomass threshold, not proof that the pre-shadow ecological state or every lineage relationship was restored.
