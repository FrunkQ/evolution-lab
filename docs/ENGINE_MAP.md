...
| Generic candidate-tuning contract and model boundary | `src/lib/calibration` | Content-hashed `TuningSpec`, candidate/evaluation/attempt records, authority/bounds/units, hard-gate-first Fitness Vector, Pareto comparison and OpenAI-compatible strict proposal parsing | `src/lib/calibration/calibration.spec.ts`, `model.spec.ts`; malformed/model-hallucinated fields and frozen/provider changes are rejected before simulation |
...
| Machine-readable tuning CLI | `scripts/tune.ts`, invoked by `npm run tune --` | Template, baseline, file evaluation and optional OpenAI-compatible model attempt JSON; endpoint configuration is environment-only | `template`, `baseline`, `evaluate`, `model`; API keys are read but never emitted or persisted, mocked connector tests remain release authority |
