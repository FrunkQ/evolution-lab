...
| Material transaction ledger and validation | `src/lib/core/accounting.ts`; transaction call sites in `src/lib/core/simulate.ts` | Versioned boundary/unit/minor-unit frames, ordered account postings, imports/exports, recomputed residuals and adjustment debt | `src/lib/core/accounting.spec.ts`, `src/lib/core/simulate.spec.ts` and `src/lib/analysis/pairedBiomass.spec.ts`; non-biological identity reuse, exact closure/continuity plus deliberate residual and debt failures |
...
| Seed derivation and random streams | `src/lib/core/rng.ts` | Run master seed, named paths and explicit counters for order-independent draws | `src/lib/core/rng.spec.ts` plus core simulation tests |
| Generic typed habitat graph | `src/lib/core/habitatGraph.ts` | Content-addressed node/link definitions with generic facts and bounded transfer fractions | `src/lib/core/habitatGraph.spec.ts`; Alien Lake is the first adapter, not a second graph authority |
| Exact retained-state wrapper and observable comparison | `src/lib/core/abstraction.ts` | Hashed boundary, sorted members, retained state, resume triggers and per-observable tolerances | `src/lib/core/abstraction.spec.ts`; this precursor does not claim compute saving or backmapping |
| Generic numeric-field response function | `src/lib/mechanisms/responseFunction.ts` | Typed coordinate/value semantics and units, bounded bands, efficiency, costs, evidence, returned contributions and hash | `src/lib/mechanisms/responseFunction.spec.ts` uses a non-spectral fixture to prevent pigment/domain coupling |
...
| Microbial reference qualification adapter | `src/lib/analysis/microbialQualification.ts` | Existing provider, reference, checkpoint, ten-gate evaluation-family, workload and causal authorities only | `src/lib/analysis/microbialQualification.spec.ts` and `npm run qualify`; pinned lightweight UI summary must equal the executable report |
...
| Microbial workload and benchmark adapter | `src/lib/analysis/microbialPerformance.ts` | Read-only `SimulationRun` counts plus opt-in fixed-seed timing callback | `src/lib/analysis/microbialPerformance.spec.ts`; seven structural limits including accounting-record retention, bounded samples and no invented population estimate |
...
| Generic candidate-tuning contract and model boundary | `src/lib/calibration`; revision controller in `src/lib/calibration/modelLoop.ts` | Content-hashed `TuningSpec`, candidate/evaluation/attempt records, authority/bounds/units, hard-gate-first Fitness Vector, Pareto comparison, structured-output negotiation and bounded calibration-only feedback | `src/lib/calibration/calibration.spec.ts`, `model.spec.ts`, `modelLoop.spec.ts`; malformed/model-hallucinated fields and frozen/provider changes are rejected, attempts are hash-chained and held-out evidence is withheld from revision prompts |
| Microbial tuning spec and evaluation adapter | `src/lib/analysis/microbialTuning.ts` | Three existing light-weaver mechanism constants, frozen provider mean light, named smoke/calibration/held-out/release seeds and real paired-run observations | `src/lib/analysis/microbialTuning.spec.ts`; default promoted hashes stay unchanged without a candidate and candidate evaluations replay exactly |
| Alien Lake integration adapter | `src/lib/analysis/alienLake.ts` | Pinned spectral fixture, authored response definitions, generic habitat graph, closed lake-material transactions, named daughter draw, exact wrapper/control and explicit counterfactuals | `src/lib/analysis/alienLake.spec.ts` and `npm run qualify`; remains draft until manifest/provider/checkpoint promotion |
| Alien Lake framework-neutral view | `src/lib/projections/alienLake.ts` | Read-only selected-day habitats, spectra/responses, material histories, event trail and scale proof | `src/lib/projections/alienLake.spec.ts`; projection choices cannot alter the run |
...
| Experiment scene view | `src/lib/projections/scene.ts` | Versioned presentation facts for microbial and Alien Lake scenarios | `ExperimentScene.svelte`; no core prose or run mutation |
| Alien Lake renderer and local three-lens explanation | `src/lib/components/AlienLakeExperiment.svelte` | One `AlienLakeView`, typed spectrum options and host callbacks | Svelte check/browser verification; native SVG shapes are labelled as normalised and exact-wrapper limits stay visible |
...
| Tuning Harness renderer | `src/lib/components/TuningHarness.svelte` | One compiled tuning spec plus a host evaluation callback; local human slider state only | Svelte check/browser verification; hard gates precede vector changes and the component cannot import simulation/domain adapters or connect to a model |
...
| Machine-readable tuning CLI | `scripts/tune.ts`, invoked by `npm run tune --` | Template, baseline, file evaluation and optional bounded OpenAI-compatible model-loop JSON; endpoint configuration is environment-only | `template`, `baseline`, `evaluate`, `model [maxAttempts]`; API keys are read but never emitted or persisted, mocked connector tests remain release authority |
