...
| Reference experiments and expected checkpoints | `src/lib/experiments/catalog.ts`; validation/hash authority in `src/lib/experiments/validate.ts` | Versioned experiment record, immutable manifest hash and expected checkpoint hashes | `src/lib/experiments/catalog.spec.ts`; exact checkpoint reproduction and silent-content-change rejection |
...
| Checkpoint microbial feedback evaluation | `src/lib/analysis/pairedBiomass.ts` | One verified parent checkpoint, aligned control/shadow futures, repeat fork and uninterrupted reference | `src/lib/analysis/pairedBiomass.spec.ts`; prefix/branch/resume integrity, metrics, generated causal steps, hard gates and unavailable-check disclosure |
